...
- Before drafting a technical article, inspect the site-wide article index and the relevant existing article bodies, including other series. Map planned derivations to their existing homes. Prefer a brief equation recall, notation/assumption alignment, and a nearby link over rederiving established material. Retain genuinely new steps and benchmarks; if an older foundation lacks necessary derivation steps, propose consolidating them there rather than silently discarding them. Do not conflate related but different objects (for example ordinary versus Kubo flux-side correlations, or classical versus centroid PMFs). A request to audit duplication does not by itself authorize rewriting or republishing article bodies.
...
- In method explanations, connect each intermediate quantity back to the requested observable before introducing another representation. A dominant exponent or stationary path alone is not a rate; identify the missing fluctuation/prefactor and reactant normalization. A DVR benchmark must actually construct and solve the discretized quantum problem; an analytic reference is a separate validation, not a relabeled DVR result. Recheck plateau windows, state cutoffs, and finite-box/grid effects whenever extending a benchmark to lower temperatures.
